...
\caption{The central puzzle: Fourth-down aggression increased dramatically (+6pp over the period), but decision \textit{quality} stayed flat ($-0.09$ pp/year). Coaches changed behavior but not accuracy.}
\end{figure}
...
\textbf{Note:} Numbers from learning\_by\_margin.csv match above. The trend $p$-values: Close ($p=0.01$), Moderate ($p=0.01$), Clear ($p=0.58$), Obvious ($p=0.66$).
...
\begin{itemize}[nosep]
    \item When model says GO: compliance improved 33\% $\to$ 51\% \hfill \textbf{+17.5 pp}
    \item When model says PUNT: compliance dropped 95\% $\to$ 90\% \hfill $-4.5$ pp
    \item When model says FG: compliance stayed flat 71\% $\to$ 71\% \hfill $+0.6$ pp
\end{itemize}
...
\caption{Two-point conversion learning curve around the 2015 rule change. Pre-rule (2013--2014), coaches were $\sim$98\% defensibly optimal. The rule change caused an immediate drop to 85\% in 2015. Coaches then recovered, reaching 99\% by 2024.}
\end{figure}
...
\begin{itemize}[nosep]
    ...
    \item Recovery: 83\% (2016) $\to$ 99\% (2024)
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Magnitude:} In 107 situations where the model said ``go for 2'' when down 9, coaches kicked the PAT in 106 of them
\end{itemize}
...
\section*{Robustness: Team-Deficit Correlation}

A potential concern with the Down 8 vs Down 9 paradox: are certain teams systematically more likely to end up in particular deficit situations? If so, team quality could confound the analysis.

\textbf{Test:} Chi-square test for independence between team identity and score deficit (Down 1, 2, or 3 after TD).
...
\midrule
Sample size & 1,439 plays \\
Chi-square & 45.61 \\
Degrees of freedom & 62 \\
P-value & \textbf{0.94} \\
Cramer's V & 0.13 (small) \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Result:} Team is \textit{not} correlated with deficit type ($p = 0.94$, Cramer's V = 0.13). Being down 2 vs down 3 after a touchdown is essentially random with respect to team identity.

\textbf{Distribution of deficits:}
\begin{itemize}[nosep]
    \item Down 1 (was down 7): 78.2\% of situations
    \item Down 2 (was down 8): 13.6\% of situations
    \item Down 3 (was down 9): 8.2\% of situations
\end{itemize}

This rules out selection effects as an explanation for the Down 8 vs Down 9 paradox. The behavioral anomaly is not driven by particular teams systematically facing one situation more than another.
