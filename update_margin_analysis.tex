...
\caption{Two-point conversion analysis around the 2015 rule change (expanding window, full hierarchical model). Pre-2015 shows $\sim$66\% optimal (stable after 3 years training data). Post-2015 shows initial drop to $\sim$43\% then gradual recovery with +1.05 pp/year trend. Right: PAT success rate dropped from $\sim$99\% to $\sim$94\%.}
\end{figure}

\textbf{Key findings:}
\begin{itemize}[nosep]
    \item \textbf{Pre-2015} (2009--2014, trained on 3+ years): Average 65.5\% optimal (range: 51--82\%)
    \item \textbf{Post-2015} (2016--2024): Average 54.2\% optimal (range: 43--58\%)
    \item Post-2015 trend: $+1.05$ pp/year (modest improvement)
\end{itemize}

The pattern is exactly what we'd expect from the rule change: coaches were reasonably calibrated pre-2015 ($\sim$66\% optimal), then \textit{dropped} dramatically in 2016 when the rule changed (43\% optimal---2016 uses only 2015 training data, noted as ex post), then gradually improved as they learned the new environment. By 2023, they're back to $\sim$58\% optimal.
