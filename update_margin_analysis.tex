...
\caption{Two-point conversion analysis around the 2015 rule change (expanding window, full hierarchical model). Post-2015 shows initial drop to $\sim$43\% then gradual recovery with +1.05 pp/year trend. Pre-2015 data shown for completeness but should be interpreted with caution---see note below. Right: PAT success rate dropped from $\sim$99\% to $\sim$94\%.}
\end{figure}

\textbf{Key findings (post-2015 only):}
\begin{itemize}[nosep]
    \item Average optimal rate: 54.2\% (range: 43--58\%)
    \item Trend: $+1.05$ pp/year (modest improvement)
    \item Coaches started at $\sim$43\% optimal in 2016 (limited training data) and have improved to $\sim$58\% by 2023
\end{itemize}

\textbf{Note on pre-2015 analysis:} We exclude the pre-2015 era from the main comparison because virtually all two-point decisions had WP margins below 0.5\% when PAT success was $\sim$99\%. The model's recommendations are essentially noise at these margins---even a 0.1\% margin threshold pushes optimal rates above 90\%. The post-2015 era, with PAT at $\sim$94\%, creates meaningful WP differences that allow for genuine evaluation of coach decision quality.
