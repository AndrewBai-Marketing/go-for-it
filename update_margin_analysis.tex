...
\caption{The central puzzle: Fourth-down aggression increased dramatically (+8pp over the period), but decision \textit{quality} stayed flat ($-0.05$ pp/year, $p = 0.54$). Coaches changed behavior but not accuracy.}
\end{figure}
...
\caption{Left: Decision quality by margin size. Most decisions (53\%) are close calls with only 69\% optimality. Right: Learning trends show coaches getting \textit{worse} on close calls and moderate decisions (red = statistically significant).}
\end{figure}
...
\caption{Two-point conversion analysis around the 2015 rule change. Left: Optimality dropped initially as coaches were calibrated to old math, then recovered as they learned. Right: Coaches remain far too conservative (actual 2pt rate ~10\% vs optimal ~45\%).}
\end{figure}

\textbf{Key insight:} The improving trend in two-point decisions (+1.0 pp/year) represents coaches slowly adapting to the post-2015 environment. The contrast with fourth down (flat trend) supports the complexity hypothesis: simpler decisions allow for faster learning.
