...
\subsection{Do Teams Recognize Kicker Value?}

A natural question is whether front offices appropriately value kicking ability. If teams understood the hierarchical kicker effects, we would expect teams with below-average kickers to replace them at higher rates.

We find mixed evidence. Among the bottom-10 kickers by effect, 7 were replaced within two seasons of their poor performance being identifiable. However, the correlation between estimated kicker effect and tenure is weak ($r = 0.12$), suggesting teams rely more on raw make percentage than properly shrunk estimates. This creates an opportunity for teams to gain marginal wins by applying Bayesian reasoning to kicker evaluation---though the total value at stake (approximately 0.1 wins per season) is modest.
