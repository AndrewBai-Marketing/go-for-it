...
\section{Team Decision Quality and Kicker Value}

\subsection{Wins Lost by Team}

We aggregate decision quality at the team level to identify which organizations make the best and worst fourth-down decisions. Table~\ref{tab:team_wins_lost} reports average wins lost per season from suboptimal decisions.

\begin{table}[H]
\centering
\caption{Wins Lost from Suboptimal Fourth Down Decisions by Team (2014--2023)}
\label{tab:team_wins_lost}
\begin{tabular}{@{}lccccc@{}}
\toprule
\textbf{Team} & \textbf{4th Downs/Yr} & \textbf{Mistakes/Yr} & \textbf{Optimal \%} & \textbf{Wins Lost/Yr} \\
\midrule
\multicolumn{5}{l}{\textit{Worst Decision-Makers}} \\
CHI & 127.3 & 38.8 & 69.5\% & 0.51 \\
NE & 122.2 & 32.3 & 73.5\% & 0.45 \\
PIT & 125.5 & 33.2 & 73.6\% & 0.44 \\
DET & 111.2 & 34.0 & 69.4\% & 0.43 \\
CAR & 130.8 & 31.8 & 75.7\% & 0.43 \\
\midrule
\multicolumn{5}{l}{\textit{Best Decision-Makers}} \\
SF & 101.3 & 27.7 & 72.7\% & 0.35 \\
GB & 104.8 & 27.2 & 74.1\% & 0.34 \\
TB & 112.3 & 27.8 & 75.2\% & 0.32 \\
BAL & 107.5 & 26.3 & 75.5\% & 0.31 \\
CIN & 115.5 & 29.2 & 74.7\% & 0.30 \\
\midrule
\textbf{League Avg} & 117.6 & 30.9 & 73.7\% & 0.39 \\
\bottomrule
\end{tabular}
\end{table}

The spread between best and worst teams is approximately 0.2 wins per season---meaningful over time but small relative to other sources of team performance variation. Notably, Chicago's poor decision-making (0.51 wins lost per year) costs roughly as much as the difference between an elite and replacement-level kicker.

\subsection{Kicker Value Rankings}

Table~\ref{tab:kicker_rankings} presents the top and bottom kickers by Bayesian-shrunk effect, with estimated 50-yard make probabilities.

\begin{table}[H]
\centering
\caption{Kicker Rankings by Bayesian Effect (2006--2024)}
\label{tab:kicker_rankings}
\begin{tabular}{@{}clccc@{}}
\toprule
\textbf{Rank} & \textbf{Kicker} & \textbf{Team} & \textbf{FGs} & \textbf{50-yd Make \%} \\
\midrule
\multicolumn{5}{l}{\textit{Top 5}} \\
1 & C. Boswell & PIT & 191 & 73.1\% \\
2 & N. Folk & TEN & 173 & 73.0\% \\
3 & C. Dicker & LAC & 97 & 72.7\% \\
4 & B. Aubrey & DAL & 85 & 72.5\% \\
5 & J. Tucker & BAL & 205 & 72.2\% \\
\midrule
\multicolumn{5}{l}{\textit{Bottom 5}} \\
61 & J. Moody & SF & 59 & 65.8\% \\
62 & M. Badgley & DET & 103 & 65.8\% \\
63 & R. Succop & TB & 105 & 65.1\% \\
64 & A. Rosas & NO & 33 & 64.6\% \\
65 & M. Ammendola & HOU & 35 & 63.9\% \\
\midrule
& \textit{League Average} & --- & --- & 69.2\% \\
\bottomrule
\end{tabular}
\end{table}

The best-to-worst spread at 50 yards is 9.2 percentage points (73.1\% vs 63.9\%). Converting to wins: an elite kicker adds approximately 0.06--0.07 wins per season relative to league average, while the worst kickers cost a similar amount. The total spread between best and worst is roughly 0.12 wins per season---smaller than the spread in team decision quality.

\subsection{Can Front Offices Identify Decision Quality?}

A natural question is whether front offices can---and do---appropriately evaluate coaching decision quality and kicker ability.

\paragraph{Kicker evaluation.} We find moderate correlation between estimated kicker effect and career tenure ($r = 0.39$), suggesting teams do partially recognize kicking ability. However, the correlation with \textit{raw} make percentage is similar ($r = 0.33$), indicating teams may rely on unshrunken statistics rather than properly pooled Bayesian estimates. Among inactive kickers, there is no significant difference in tenure between those with positive effects (2.2 years) and negative effects (2.7 years, $p = 0.46$).

\paragraph{Coaching decision quality.} The challenge of evaluating fourth-down decision-making is more severe. Year-over-year team decision quality shows:
\begin{itemize}
    \item Near-zero autocorrelation ($r = -0.08$), indicating decision quality is not persistent
    \item Strong mean reversion ($r = -0.69$ between prior year rate and change)
    \item Bottom-quartile teams improve by +5.4 pp on average; top-quartile teams decline by -5.4 pp
\end{itemize}

This pattern suggests fourth-down decision quality is dominated by noise rather than stable coaching skill. A team that appears to make poor decisions one year is likely to improve the next---not because they fired their coach or changed their approach, but because they regressed to the mean.

\paragraph{Implications.} The low persistence of decision quality creates a fundamental identification problem: front offices cannot reliably distinguish good from bad fourth-down decision-makers based on observed outcomes. This may explain why coaching changes do not appear correlated with fourth-down performance---there is simply too much noise to draw valid inferences. By contrast, kicker ability shows more persistence, making it a more viable target for analytical evaluation.

\section{Game-Theoretic Analysis of Run/Pass Decisions}

We extend the decision framework beyond fourth downs to analyze the fundamental run/pass tradeoff. While fourth-down decisions represent discrete, high-stakes choices, run/pass decisions occur on nearly every play and offer a cleaner test of whether teams have learned to optimize over time.

\subsection{The 2$\times$2 Game}

We model play-calling as a simultaneous game between offense (choosing run or pass) and defense (choosing box personnel). Let:
\begin{itemize}
    \item \textbf{Offense actions:} $\{$run, pass$\}$
    \item \textbf{Defense actions:} $\{$light box ($\leq 6$ defenders), heavy box ($\geq 7$ defenders)$\}$
\end{itemize}

The payoff matrix (EPA per play, offense perspective) estimated from 309,950 plays (2016--2024):

\begin{table}[H]
\centering
\caption{Run/Pass Payoff Matrix (EPA)}
\begin{tabular}{@{}lcc@{}}
\toprule
& \textbf{Light Box} & \textbf{Heavy Box} \\
\midrule
\textbf{Run} & $+0.021$ & $-0.068$ \\
\textbf{Pass} & $-0.003$ & $+0.083$ \\
\bottomrule
\end{tabular}
\end{table}

The structure reveals the game's logic: running is profitable against light boxes but costly against heavy boxes; passing is slightly negative against light boxes but highly profitable against heavy boxes. Neither action dominates, motivating mixed strategies.

\subsection{Nash Equilibrium}

In the mixed strategy Nash equilibrium, each player randomizes to make the opponent indifferent. Solving the indifference conditions:
\begin{align}
    p^* &= \frac{(\text{R}_H - \text{R}_L)}{(\text{R}_H - \text{R}_L) - (\text{P}_H - \text{P}_L)} = 50.9\% \quad \text{(optimal pass rate)} \\
    q^* &= \frac{(\text{P}_L - \text{R}_L)}{(\text{R}_H - \text{R}_L) - (\text{P}_H - \text{P}_L)} = 13.6\% \quad \text{(optimal heavy box rate)}
\end{align}

The key empirical finding: \textbf{Teams pass too much.} The actual pass rate is 58.7\% versus the optimal 50.9\%---a gap of 7.8 percentage points. Defenses also over-commit to heavy boxes (39.8\% vs optimal 13.6\%), but this may be a rational best response to the high observed pass rate.

\subsection{Has the Gap Closed Over Time?}

We analyze whether teams have learned to optimize by comparing actual vs. optimal pass rates across eras:

\begin{table}[H]
\centering
\caption{Run/Pass Optimization by Era}
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Era} & \textbf{Actual Pass \%} & \textbf{Optimal Pass \%} & \textbf{Gap} & \textbf{N Plays} \\
\midrule
2016--2018 & 59.4\% & 52.0\% & $-7.4$ pp & 100,475 \\
2019--2021 & 59.0\% & 52.1\% & $-6.9$ pp & 103,423 \\
2022--2024 & 57.6\% & 49.0\% & $-8.6$ pp & 106,052 \\
\bottomrule
\end{tabular}
\end{table}

The evidence does not support learning: the gap has widened from 7.4 pp to 8.6 pp. Pass rates have declined slightly (59.4\% $\to$ 57.6\%), but optimal rates have declined faster (52.0\% $\to$ 49.0\%) as defenses have adjusted. The year-over-year trend shows no significant improvement ($+0.59$ pp/year, $p = 0.40$).

\subsection{Heterogeneity by Game State}

The pass rate gap varies substantially by situation:

\begin{table}[H]
\centering
\caption{Pass Rate Gap by Down and Distance}
\begin{tabular}{@{}llccc@{}}
\toprule
\textbf{Down} & \textbf{Distance} & \textbf{Actual} & \textbf{Optimal} & \textbf{Gap} \\
\midrule
1st & Long (7--10) & 49.1\% & 45.5\% & $-3.6$ pp \\
2nd & Medium (4--6) & 52.1\% & 100.0\% & $+47.9$ pp \\
2nd & Long (7--10) & 65.1\% & 69.8\% & $+4.8$ pp \\
3rd & Short (1--3) & 51.8\% & 78.2\% & $+26.3$ pp \\
4th & Short (1--3) & 46.0\% & 97.8\% & $+51.7$ pp \\
\bottomrule
\end{tabular}
\end{table}

The largest inefficiencies occur on 2nd-and-medium and short-yardage situations, where the model recommends much more frequent passing than observed. On 2nd-and-4-to-6, passing is essentially always optimal (against the observed defensive alignments), yet teams pass only 52\% of the time.

\subsection{EPA Left on the Table}

The efficiency cost of suboptimal mixing is approximately 0.009 EPA per play. Over a 17-game season with roughly 1,000 offensive plays, this translates to approximately 9 EPA left on the table---equivalent to roughly 0.25 wins per season. This is smaller than the cost of fourth-down mistakes (0.39 wins/year) but affects far more plays.

\subsection{Interpretation}

The run/pass analysis complements the fourth-down findings in several ways:

\begin{enumerate}
    \item \textbf{No evidence of learning:} Teams have not converged toward optimal mixing over the 2016--2024 period. If anything, the gap has widened as defenses have adjusted.

    \item \textbf{Direction of bias differs from fourth down:} Teams are too \textit{conservative} on fourth down (not going for it enough) but too \textit{aggressive} with passing (passing too much relative to equilibrium). This suggests the biases are not simply ``always do the safe thing'' but reflect more complex situational heuristics.

    \item \textbf{Defenses respond:} The high heavy box rate (39.8\% vs optimal 13.6\%) may be a rational response to offensive over-passing. This creates a feedback loop: teams pass too much $\to$ defenses stack the box less $\to$ running becomes more profitable $\to$ optimal pass rate falls $\to$ gap widens.

    \item \textbf{Large situational inefficiencies:} The biggest gains are available on 2nd-and-medium and short-yardage situations, where conventional wisdom (``establish the run'') conflicts with equilibrium analysis.
\end{enumerate}
