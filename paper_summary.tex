...
\begin{keyresult}
\textbf{Coaches remain systematically too conservative on fourth down.} Using nflfastR's win probability model with state-transition calculations:
\begin{itemize}[nosep]
  \item Optimal GO rate: \textbf{46.6\%}---but coaches only go 14.8\%
  \item 4th \& 1 optimal GO rate: \textbf{70.8\%}---coaches go only $\sim$25\%
  \item Match rate: \textbf{52.5\%}---coaches make the wrong call roughly half the time
  \item Teams lose \textbf{0.27 expected wins/season} from fourth down errors
\end{itemize}
\textbf{Key finding:} Coaches are \textit{too conservative}---the model says GO nearly half the time, but coaches go only 15\%. WPA analysis validates this: when the model recommends GO but teams don't, they lose an average of \textbf{$-0.011$ WPA} per play.

\textbf{Contrast with two-point conversions:} Coaches \textit{are} improving (+1.1 pp/year, REG season trend).

\textbf{Implication:} Despite 20 years of analytics research, teams continue to leave wins on the table through excessive conservatism on fourth down.
\end{keyresult}
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Bayesian logistic WP model} with home field advantage (10 features)
...
\begin{itemize}[nosep]
  ...
  \item $W(s)$: \textbf{Bayesian logistic regression} with 10 features including home field advantage (is\_home coef: 0.33) \new
\end{itemize}
...
\midrule
2006 & 3,829 & 12.1\% & 76.1\% & 70.1\% & 90.7\% \\
2010 & 3,840 & 12.5\% & 77.8\% & 72.1\% & 91.6\% \\
2015 & 3,824 & 12.4\% & 73.2\% & 72.9\% & 96.2\% \\
2020 & 3,402 & 19.3\% & 76.2\% & 72.3\% & 92.4\% \\
2024 & 3,827 & 20.0\% & 72.6\% & 72.5\% & 95.2\% \\
\midrule
\textit{Overall} & 71,786 & 14.8\% & \textbf{75.9\%} & 72.7\% & \textbf{93.6\%} \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Key finding:} WPA analysis validates model recommendations. When the model says GO and teams follow: \textbf{+0.006 WPA}. When the model says GO but teams don't: \textbf{$-0.001$ WPA}.

\subsection*{Trend Analysis}

\textbf{Coaches remain too conservative:}
\begin{itemize}[nosep]
  \item Go-for-it rate increased: 12.1\% (2006) $\to$ 20.0\% (2024)
  \item But optimal GO rate is \textbf{46.6\%}---coaches still far too conservative
  \item 4th \& 1: Model says GO 70.8\% of the time, coaches go only $\sim$25\%
  \item Conservative errors dominate: teams punt/kick when going for it is optimal
\end{itemize}

\textbf{WPA validates the model:}
\begin{itemize}[nosep]
  \item When model says GO and teams go: +0.0059 WPA (positive outcome)
  \item When model says GO but teams don't: $-0.0009$ WPA (value left on table)
  \item 4th \& 1 specifically: +0.0125 WPA when following GO vs $-0.0108$ when ignoring
\end{itemize}
...
\midrule
Close call ($<$2\% margin) & 11,797 & 16.4\% & 85.7\% \\
Moderate (2--5\% margin) & 1,795 & 2.5\% & 13.0\% \\
Clear ($>$5\% margin) & 167 & 0.2\% & 1.2\% \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Finding:} 86\% of ``mistakes'' are close calls where margin $<$ 2\%. Only 1.2\% of mistakes are clear errors. Most disagreements are \textit{defensible}.
...
\midrule
CLE (worst) & 0.34 & 0.37 & 0.38 & 0.25 & 0.76 & \textbf{0.34} \\
NYJ & 0.12 & 0.19 & 0.25 & 0.69 & 0.51 & 0.33 \\
TEN & 0.31 & 0.32 & 0.31 & 0.40 & 0.31 & 0.33 \\
\midrule
SF (best) & 0.28 & 0.25 & 0.28 & 0.13 & 0.37 & \textbf{0.22} \\
NO & 0.17 & 0.25 & 0.15 & 0.31 & 0.27 & 0.22 \\
DAL & 0.24 & 0.29 & 0.30 & 0.20 & 0.31 & 0.23 \\
\midrule
\textit{League Avg} & 0.27 & 0.30 & 0.34 & 0.34 & 0.38 & 0.27 \\
\bottomrule
\end{tabular}
\end{center}
\end{contributionbox}
...
\begin{itemize}[nosep]
  \item 2006--2012 average: 0.25 wins/team/season
  \item 2020--2024 average: 0.33 wins/team/season
  \item 2024: 12.3 total league wins lost (vs 8.2 in 2006)
\end{itemize}
...
2024 & CHI @ SEA & 4th \& 10, opp 40, $-3$, 0:20 Q4 & Go & FG & 19.3\% \\
2009 & --- & 4th \& 10, opp 40, tied, 0:19 Q4 & Punt & FG & 14.6\% \\
2024 & ATL @ NO & 4th \& 4, opp 43, $-3$, 0:07 Q4 & Go & FG & 10.0\% \\
2019 & TEN @ IND & 4th \& 2, opp 45, $-2$, 0:13 Q4 & Go & FG & 9.6\% \\
\bottomrule
\end{tabular}
\end{center}
\normalsize
...
\begin{itemize}[nosep]
  \item Going for it when a field goal would have tied/won (BUF at TEN 3-yard line)
  \item Going for it or punting late in close games instead of attempting a makeable FG
  \item Maximum WP cost of 34.3\% reflects a game-deciding situation
\end{itemize}
...
Bayesian logistic WP model with home field advantage & \new \\
Expanding window (ex ante vs ex post) design & \new \\
WPA validation of model recommendations & \new \\
Coaches remain systematically too conservative & \new \\
Optimal GO rate (46.6\%) vs actual (14.8\%) gap documented & \new \\
...
\begin{itemize}[nosep]
  ...
  \item Teams with high variance in decision quality (CLE, NYJ, TEN) lose more from errors
  \item Coaches remain too conservative: optimal GO rate is 46.6\%, actual is 14.8\%
  \item WPA evidence validates aggressive play: +0.006 WPA when following GO recommendations
\end{itemize}
...
\begin{tabular}{lcccc}
\toprule
\textbf{Situation} & \textbf{N} & \textbf{Model: 2pt} & \textbf{Actual 2pt} & \textbf{Compliance} \\
\midrule
Down 8 $\to$ Down 2 & 196 & 85\% & \textbf{79\%} & \textbf{79\%} \\
Down 9 $\to$ Down 3 & 118 & 91\% & 1\% & \textbf{10\%} \\
\midrule
Down 14 $\to$ Down 8 & 448 & 94\% & 8\% & 15\% \\
Down 15 $\to$ Down 9 & 97 & 99\% & 23\% & 24\% \\
Down 7 $\to$ Down 1 & 1,125 & 67\% & 3\% & 33\% \\
Tied $\to$ Up 6 & 2,582 & 42\% & 1\% & 58\% \\
\bottomrule
\end{tabular}
\end{center}
\end{contributionbox}
...
\caption{The two-point conversion paradox. Blue bars show the model-optimal rate; red bars show actual coach behavior. Down 9 has a \textit{higher} optimal 2pt rate than Down 8 (91\% vs.\ 85\%), yet coaches comply 79\% of the time when down 8 and only 10\% when down 9.}
...
\textbf{Magnitude:} In 107 situations where the model said ``go for 2'' when down 9, coaches kicked the PAT in 106 of them. This is not a close call---it's a systematic blind spot.
