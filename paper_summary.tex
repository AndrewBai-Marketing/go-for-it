...
\begin{itemize}[nosep]
  \item $\pi(d; \theta)$: Hierarchical logistic regression on yards to go \known
  \item $Y(x; \theta)$: Normal model for net punt yards (mean $\approx$ 40 yds) \known
  \item $\phi(x; \theta)$: Logistic regression on kick distance \known
