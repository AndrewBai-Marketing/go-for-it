...
\begin{abstract}
We develop a Bayesian decision-theoretic framework for analyzing fourth down decisions in American football. Unlike existing approaches that rely on point estimates of win probability, our framework explicitly accounts for parameter uncertainty by integrating over posterior distributions of transition probabilities. We extend the baseline models using hierarchical Bayes to capture \textit{both} offensive team conversion ability and defensive team stopping ability, as well as kicker-specific field goal accuracy, with empirical Bayes shrinkage ensuring stable estimates. Critically, our model incorporates \textit{asymmetric clock consumption}: converting a fourth down burns approximately 150 seconds until the next change of possession (retaining the ball), while failing burns only 48 seconds---a distinction essential for late-game analysis. This allows us to make a selection-on-observables argument: by conditioning on the matchup-specific factors that coaches observe at decision time, we claim our model captures the coach's decision-relevant information set. Applying this framework to 71,786 fourth-down situations from 2006--2024, we find that coaches deviate from model recommendations approximately 19.5\% of the time---but critically, 83\% of these deviations are ``close calls'' where the decision margin is less than 2 percentage points of win probability. Only 0.5\% of all plays represent clear mistakes where the coach should unambiguously have known better. Using an expanding window methodology with a 7-year minimum training period starting from 1999, we show that 96.5\% of optimal decisions were \textit{knowable in real-time}. Despite the analytics revolution increasing league-wide go-for-it rates from 12.5\% to 20.0\% between 1999 and 2024, we find no improvement in decision quality---coaches have simply shifted from excessive conservatism to excessive aggression, leaving expected ``free wins'' still on the table. The fact that coaches do not outperform model recommendations, and that playoff decisions are no better than regular season decisions despite higher stakes, supports our claim that coach suboptimality reflects behavioral biases rather than private information.
\end{abstract}
...
\end{equation}
where $\Delta$ is score differential, $\tau$ is seconds remaining, $x$ is yards from opponent's end zone, and $k$ is timeout differential. The negative interaction term ($\hat{\beta}_3 = -2.647$) confirms that score differential matters more as time decreases.
...
\midrule
2006 & 3,829 & 12.1\% & 82.0\% & 78.5\% & 93.5\% \\
2007 & 3,758 & 14.1\% & 77.5\% & 77.3\% & 95.5\% \\
2008 & 3,674 & 13.2\% & 80.3\% & 78.7\% & 95.5\% \\
2009 & 3,849 & 14.4\% & 78.8\% & 78.1\% & 96.3\% \\
2010 & 3,840 & 12.5\% & 82.4\% & 79.2\% & 94.6\% \\
2011 & 3,845 & 11.1\% & 82.2\% & 81.1\% & 95.1\% \\
2012 & 3,820 & 11.6\% & 82.6\% & 80.3\% & 96.4\% \\
2013 & 3,904 & 11.9\% & 83.1\% & 81.2\% & 96.9\% \\
2014 & 3,740 & 12.0\% & 80.1\% & 80.2\% & 96.1\% \\
2015 & 3,824 & 12.4\% & 79.5\% & 80.7\% & 96.8\% \\
2016 & 3,703 & 12.8\% & 82.1\% & 80.9\% & 97.7\% \\
2017 & 3,869 & 12.4\% & 82.6\% & 81.2\% & 98.3\% \\
2018 & 3,574 & 14.9\% & 81.4\% & 81.5\% & 97.0\% \\
2019 & 3,634 & 16.3\% & 79.9\% & 80.0\% & 98.8\% \\
2020 & 3,402 & 19.3\% & 80.5\% & 79.9\% & 97.3\% \\
2021 & 3,777 & 20.9\% & 79.0\% & 78.8\% & 95.7\% \\
2022 & 3,870 & 18.9\% & 78.9\% & 78.4\% & 96.9\% \\
2023 & 4,047 & 19.6\% & 77.9\% & 78.7\% & 96.5\% \\
2024 & 3,827 & 20.0\% & 78.9\% & 79.0\% & 97.9\% \\
\midrule
\textbf{Overall} & \textbf{71,786} & \textbf{15.3\%} & \textbf{80.5\%} & \textbf{79.7\%} & \textbf{96.5\%} \\
\bottomrule
\end{tabular}
\end{table}
...
Applying this framework to the 13,990 plays where coaches deviated from model recommendations:
...
Clear mistake & 674 & 0.9\% & Margin $>$ 5\% WP \\
\midrule
...
This decomposition reveals a striking fact: \textbf{83\% of ``mistakes'' are actually close calls}. When coaches deviate from model recommendations, they overwhelmingly do so in situations where the data do not clearly favor one action. Only 0.9\% of all fourth-down plays---and just 4.8\% of deviations---represent clear errors where the coach should unambiguously have known better
...
2016 BAL @ JAX & 4th \& 2, own 49, down 1, 2:14 Q4 & GO & PUNT & 26.8\% \\
2020 DAL @ SEA & 4th \& 3, own 47, down 1, 2:37 Q4 & GO & PUNT & 26.7\% \\
2012 DAL @ CAR & 4th \& 1, own 40, down 2, 2:11 Q4 & GO & PUNT & 26.0\% \\
2020 DEN @ NYJ & 4th \& 3, own 50, down 2, 2:05 Q4 & GO & PUNT & 24.8\% \\
\bottomrule
\end{tabular}
\end{table}

\begin{remark}[Interpreting the Worst Decisions]
The costliest mistake in our sample is the 2021 Bills @ Titans game, where Buffalo went for it on 4th \& 1 from the Tennessee 3-yard line with 22 seconds remaining while trailing by 3. The model strongly favors a field goal to tie---converting gives only 12.5\% win probability (not enough time for another possession even if successful), while a 21-yard field goal to force overtime yields 46.9\% win probability. Josh Allen was stopped for no gain, and the Bills lost.

The remaining entries share a common pattern: going for it from one's own territory in the final 2--3 minutes when trailing by 1--3 points. In these situations, the model strongly prefers punting: a failed conversion gives the opponent excellent field position to run out the clock or kick a field goal, while a successful punt forces the opponent to execute a longer drive with time pressure. The 26--27\% margins reflect the high leverage of these situations---a single decision can swing win probability by a quarter.
\end{remark}

\begin{remark}[Clock-Aware Model: End-of-Game Scenarios]
Our model incorporates asymmetric clock consumption---converting burns $\sim$151 seconds while failing burns $\sim$48 seconds---which is essential for late-game analysis. This allows the model to naturally handle end-of-game scenarios without hard-coded filters. When a team is trailing with little time remaining, the model correctly recognizes that burning clock (by converting and running additional plays) hurts their chances, while going for it and failing preserves more time.

However, one limitation remains: in situations where a trailing team could kick a field goal to cut the deficit to 1--2 points with under 2 minutes remaining, the opponent can simply \textit{kneel out the clock}. The model's win probability calculations assume the opponent will run normal plays after receiving possession, but a team leading by any margin with insufficient time remaining can run out the clock without ever facing offensive risk.

A famous historical example is Andy Reid's punt in the 2006 NFC Divisional Playoff. Trailing the Saints 27--24 with 1:56 remaining at their own 39-yard line facing 4th \& 15, Reid elected to punt. The Saints simply kneeled out the clock from their own 22-yard line. Punting was effectively 0\% WP, while going for it---even with just 15\% conversion probability---preserved some positive chance of winning. This edge case illustrates that while our clock model substantially improves end-of-game analysis, extreme clock-out scenarios require additional consideration.
\end{remark}

\subsection{Model Validation: Comparison with Next Gen Stats}

As a validation check, we compared our model's recommendations against NFL Next Gen Stats (NGS), a widely-cited industry analytics source. A notable disagreement arises on the top entry in Table~\ref{tab:worst_decisions}: the 2021 BUF @ TEN game where Buffalo went for it on 4th \& 1 from the Tennessee 3-yard line with 22 seconds remaining while trailing by 3.

Our model strongly favors a field goal to tie: $\E[W \mid \texttt{fg}] = 46.9\%$ versus $\E[W \mid \texttt{go}] = 12.5\%$. NGS reportedly recommended going for it. Which analysis is correct?

\paragraph{The arithmetic strongly supports our model.} The key insight is that even \textit{after converting}, Buffalo's win probability is low. With 22 seconds remaining and no timeouts, scoring a touchdown from the 3-yard line is far from automatic. Historical data from 2006--2024 shows that teams trailing by 1--7 points with 10--30 seconds remaining at the opponent's 30--40 yard line score a touchdown only 8\% of the time, kick a field goal 39\% of the time, and fail to score 53\% of the time.

A back-of-the-envelope calculation using these historical rates:
\begin{align}
\E[W \mid \texttt{convert}] &= 0.08 \times 0.95 + 0.39 \times 0.50 + 0.53 \times 0.02 \approx 28\% \\
\E[W \mid \texttt{go}] &= 0.66 \times 0.28 + 0.34 \times 0.02 \approx 19\% \\
\E[W \mid \texttt{fg}] &= 0.66 \times 0.50 + 0.34 \times 0.02 \approx 34\%
\end{align}
where we use 66\% conversion probability (4th \& 1 historical average) and 66\% field goal make probability (53-yard historical average). The FG advantage is approximately 15 percentage points---directionally consistent with our model's 34\% margin.

\paragraph{Reverse-engineering the NGS recommendation.} For NGS's reported ``go for it'' recommendation to be correct, at least one of the following would need to hold:
\begin{itemize}
    \item Win probability after converting $\approx$ 94\% (actual: $\sim$28\%)
    \item TD scoring rate from 35 yards with 20 seconds $\approx$ 60\% (actual: 8\%)
    \item Fourth \& 1 conversion rate $\approx$ 125\% (impossible)
\end{itemize}

None of these assumptions is plausible. The historical data unambiguously supports our model's recommendation. We conjecture that NGS's calculation may not properly account for the extremely low probability of scoring a touchdown with so little time remaining, effectively treating ``conversion'' as equivalent to ``touchdown.''

\paragraph{Implications.} This validation exercise demonstrates that our framework produces recommendations grounded in historical data and basic probability theory. When model outputs disagree with external sources, we can decompose the decision into verifiable components. In this case, the disagreement stems from a fundamental question---``what is the probability of scoring a touchdown from the 35-yard line with 20 seconds remaining?''---and the historical answer (8\%) strongly favors the field goal.
...
The paradox: when down 8 (trailing by 2 after the touchdown), coaches go for 2 at an 81\% rate---\textit{higher} than the model-optimal rate of 72\%. But when down 9 (trailing by 3 after the touchdown), coaches go for 2 only 1\% of the time, despite an identical 72\% optimal rate. In 76 situations where the model recommended going for 2 when down 9, coaches kicked the PAT \textit{every single time}.
...
The down 7 situation (trailing by 1 after TD) shows similar conservatism: going for 2 avoids overtime if the opponent scores a touchdown, but coaches attempt it only 4\% of the time despite a 59\% optimal rate. Again, the deferred and probabilistic nature of the benefit---avoiding a \textit{possible future} overtime---fails to motivate action.
