...
% Title
\title{\textbf{Bayesian Decision Theory for Fourth Down Decisions} \\
\large A Framework for Optimal Play-Calling Under Model Uncertainty}
\author{}
\date{January 2026}
...
\begin{abstract}
We develop a Bayesian decision-theoretic framework for analyzing fourth down decisions in American football. Unlike existing approaches that rely on point estimates of win probability, our framework explicitly accounts for parameter uncertainty by integrating over posterior distributions of transition probabilities. We extend the baseline models using hierarchical Bayes to capture team-specific conversion rates and kicker-specific field goal accuracy, with empirical Bayes shrinkage ensuring stable estimates even for teams and kickers with limited observations. Applying this framework to 30,025 fourth-down situations from 2017--2024, we find that coaches make suboptimal decisions approximately 24\% of the time, costing an average of 0.33 percentage points of win probability per decision. Using an expanding window methodology, we show that 96.2\% of optimal decisions were \textit{knowable in real-time}---the recommendation using only pre-season data matches the full-sample recommendation. This undermines claims that coaches lacked sufficient information to make better decisions.
\end{abstract}
...
The objective is to maximize the probability of winning the game. Let $W: \calS \to [0, 1]$ denote the win probability function. We model this as:
\begin{equation}
    W(s) = \sigma\left(\beta_1 \Delta + \beta_2 \tau + \beta_3 (\Delta \cdot \tau) + \beta_4 x + \beta_0\right)
\end{equation}
...
The interaction term $\beta_3 (\Delta \cdot \tau)$ captures the intuition that score differential matters more when less time remains---being down by 7 with 2 minutes left is worse than being down by 7 with 28 minutes left.

We estimate $\beta$ by maximum likelihood on historical play-by-play data, yielding posterior distribution $p(\beta \mid \calD)$ via Bayesian inference with prior $\beta \sim \mathcal{N}(0, \sigma^2 I)$.

\section{Bayesian Decision Framework}

\subsection{Expected Win Probability}
...
This probability can be estimated by Monte Carlo: draw $\theta^{(m)} \sim p(\theta \mid \calD)$ for $m = 1, \ldots, M$, compute win probabilities under each draw, and calculate the fraction of draws for which the action is optimal.

\begin{remark}
Situations with $\Prob(\texttt{go} \text{ is optimal}) \approx 1$ are \textbf{obvious} go-for-it decisions; situations with $\Prob(\texttt{go} \text{ is optimal}) \approx 0.5$ are \textbf{close calls} where the data do not clearly favor one action.
\end{remark}
...
We use play-by-play data from the 2006--2024 NFL seasons obtained via the \texttt{nflfastR} package. For the expanding window analysis, we use 2006--2016 as the initial training period and evaluate decisions in 2017--2024. The evaluation sample contains 30,025 fourth-down situations.

\subsection{Bayesian Estimation}

We estimate all models using Laplace approximation to the posterior. For logistic regression, we find the MAP estimate via BFGS optimization, then approximate the posterior as Gaussian with covariance equal to the inverse Hessian at the mode. For linear regression (punt model), the posterior is exact under conjugate Gaussian priors. We use weakly informative priors $\beta \sim \mathcal{N}(0, 100)$ on all coefficients.

\subsection{Parameter Estimates}

\paragraph{Conversion probability.} We model conversion as logistic in yards to go:
\begin{equation}
    \Prob(\text{convert} \mid d) = \sigma(\alpha + \beta d)
\end{equation}
The posterior estimates are $\hat{\alpha} = 0.784$ (SE: 0.046) and $\hat{\beta} = -0.176$ (SE: 0.009), with posterior correlation $-0.72$. This yields:
\begin{align}
    \pi(1) &= 64.8\% \quad [62.9\%, 66.4\%] \\
    \pi(2) &= 60.7\% \quad [58.9\%, 62.2\%] \\
    \pi(3) &= 56.4\% \quad [54.8\%, 57.9\%] \\
    \pi(5) &= 47.6\% \quad [46.0\%, 49.3\%] \\
    \pi(10) &= 27.4\% \quad [24.9\%, 30.4\%]
\end{align}
where brackets denote 95\% credible intervals.

\paragraph{Field goal probability.} We model make probability as logistic in kick distance, centered at 35 yards:
\begin{equation}
    \Prob(\text{make} \mid d) = \sigma(\alpha + \beta (d - 35))
\end{equation}
The posterior estimates are $\hat{\alpha} = 2.383$ (SE: 0.056) and $\hat{\beta} = -0.105$ (SE: 0.004). This yields:
\begin{align}
    \phi(25) &= 96.9\% \quad [96.3\%, 97.4\%] \\
    \phi(35) &= 91.6\% \quad [90.7\%, 92.4\%] \\
    \phi(45) &= 79.1\% \quad [77.9\%, 80.3\%] \\
    \phi(52) &= 64.5\% \quad [62.1\%, 66.6\%]
\end{align}

\paragraph{Punt distance.} We model net punt yards as linear in field position with Gaussian errors:
\begin{equation}
...
\paragraph{Win probability.} We model win probability as logistic in game state features:
\begin{equation}
...
\end{equation}
where $\Delta$ is score differential, $\tau$ is seconds remaining, $x$ is yards from opponent's end zone, and $k$ is timeout differential. The posterior estimates are:
\begin{center}
\begin{tabular}{lcc}
\toprule
\textbf{Coefficient} & \textbf{Estimate} & \textbf{SE} \\
\midrule
Intercept ($\beta_0$) & 0.450 & 0.026 \\
Score differential ($\beta_1$) & 4.112 & 0.064 \\
Time remaining ($\beta_2$) & $-0.304$ & 0.040 \\
Score $\times$ time ($\beta_3$) & $-3.438$ & 0.114 \\
Field position ($\beta_4$) & $-0.445$ & 0.023 \\
Timeout differential ($\beta_5$) & 0.295 & 0.044 \\
\bottomrule
\end{tabular}
\end{center}

The negative interaction term ($\beta_3 < 0$) confirms that score differential matters more as time decreases---being down 7 with 2 minutes left is worse than being down 7 with 28 minutes left.

\section{Hierarchical Models with Team and Kicker Effects}

The baseline models treat all teams and kickers as exchangeable. In practice, some teams convert fourth downs more reliably than others, and some kickers are more accurate than league average. We extend the framework using hierarchical Bayesian models that allow for team- and kicker-specific effects while borrowing strength across units via partial pooling.

\subsection{Hierarchical Conversion Model}

We extend the conversion model to include team-specific random effects:
\begin{equation}
    \Prob(\text{convert} \mid d, \text{team} = k) = \sigma(\alpha + \beta d + \gamma_k)
\end{equation}
where $\gamma_k \sim \mathcal{N}(0, \tau^2)$ is the team-specific effect and $\tau^2$ is the between-team variance estimated via empirical Bayes.

The shrinkage estimator for team $k$ is:
\begin{equation}
    \hat{\gamma}_k = (1 - B_k) \cdot \hat{\gamma}_k^{\text{raw}}
\end{equation}
where $B_k = \frac{\text{SE}_k^2}{\text{SE}_k^2 + \tau^2}$ is the shrinkage factor. Teams with fewer observations shrink more toward the league average; teams with many observations retain estimates closer to their raw performance.

\paragraph{Estimated team effects.} The between-team variance is $\hat{\tau}^2 = 0.024$, implying a standard deviation of approximately 0.15 in log-odds. Selected team effects (2006--2024):
\begin{center}
\begin{tabular}{lcc}
\toprule
\textbf{Team} & \textbf{Effect ($\hat{\gamma}_k$)} & \textbf{N Attempts} \\
\midrule
PHI & $+0.18$ & 412 \\
DET & $+0.14$ & 389 \\
BAL & $+0.11$ & 356 \\
...
The Eagles and Lions---teams known for aggressive coaching---show positive effects, suggesting their fourth-down success is not merely due to attempting more conversions but also executing them at above-average rates.

\subsection{Hierarchical Field Goal Model}

Similarly, we model kicker-specific effects on field goal probability:
\begin{equation}
...
\paragraph{Estimated kicker effects.} The between-kicker variance is $\hat{\tau}^2 = 0.031$, implying meaningful heterogeneity in kicker ability. Selected effects:
\begin{center}
\begin{tabular}{lccc}
\toprule
\textbf{Kicker} & \textbf{Effect ($\hat{\gamma}_j$)} & \textbf{50-yd Make \%} & \textbf{N Attempts} \\
\midrule
J. Tucker & $+0.42$ & 72.1\% & 298 \\
H. Butker & $+0.31$ & 68.5\% & 187 \\
C. Boswell & $+0.28$ & 67.4\% & 203 \\
\midrule
League Avg & $0.00$ & 58.2\% & --- \\
\midrule
N. Folk & $-0.19$ & 51.2\% & 156 \\
\bottomrule
\end{tabular}
\end{center}

Justin Tucker's estimated effect implies he makes 50-yard field goals at approximately 14 percentage points higher than league average---a substantial difference that meaningfully affects optimal decision-making at the margin.

\subsection{Implications for Decision Analysis}

The hierarchical models allow situation-specific recommendations. Consider 4th \& 3 at the opponent's 35-yard line:
\begin{itemize}
    \item For a team with $\hat{\gamma}_k = +0.15$ and a kicker with $\hat{\gamma}_j = +0.30$: the decision is closer because both conversion and field goal success are above average
    \item For a team with $\hat{\gamma}_k = -0.10$ and a league-average kicker: going for it becomes less attractive relative to the field goal
\end{itemize}

In practice, the hierarchical effects rarely flip the optimal decision (they do so in approximately 3\% of cases), but they substantially affect decision confidence and the magnitude of expected win probability gains.

\section{Real-Time Knowability: Expanding Window Analysis}

A natural objection to retrospective decision analysis is that coaches could not have known the optimal decision at the time. Perhaps the models we use today rely on information that was not available in, say, 2017. This section addresses this concern directly.

\subsection{Methodology}

We implement an \textit{expanding window} analysis. For each test year $Y \in \{2017, \ldots, 2024\}$:
\begin{enumerate}
    \item Train all Bayesian models on data from 2006 through $Y-1$ only (the ``ex ante'' model)
    \item Train models on the full sample 2006--2024 (the ``ex post'' model)
...
If the models agree, the correct decision was \textit{knowable in real-time}---the coach had access to sufficient historical data to identify the optimal action. If they disagree, we cannot definitively fault the coach, since the data available at the time pointed to a different conclusion.

\subsection{Results}

\begin{table}[H]
\centering
\caption{Expanding Window Analysis: Ex Ante vs. Ex Post Model Agreement}
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Season} & \textbf{N Plays} & \textbf{Ex Ante Optimal \%} & \textbf{Ex Post Optimal \%} & \textbf{Agreement Rate} \\
\midrule
2017 & 3,874 & 76.9\% & 74.8\% & 96.6\% \\
2018 & 3,578 & 78.4\% & 76.2\% & 96.1\% \\
2019 & 3,637 & 77.3\% & 74.4\% & 92.7\% \\
2020 & 3,405 & 75.6\% & 73.8\% & 97.2\% \\
2021 & 3,778 & 74.2\% & 73.2\% & 97.4\% \\
2022 & 3,872 & 75.6\% & 73.3\% & 95.7\% \\
2023 & 4,053 & 75.4\% & 74.0\% & 97.1\% \\
2024 & 3,828 & 75.7\% & 74.5\% & 96.7\% \\
\midrule
\textbf{Overall} & \textbf{30,025} & \textbf{76.1\%} & \textbf{74.3\%} & \textbf{96.2\%} \\
\bottomrule
\end{tabular}
\end{table}

The key finding: \textbf{96.2\% of plays have stable optimal decisions}. The ex ante model (trained only on data available before the season) and the ex post model (trained on all data through 2024) agree on the optimal action in the vast majority of cases.
...
\begin{itemize}
    \item 28,888 plays (96.2\%) had stable optimal decisions
    \item Of these, coaches made the optimal decision on 22,031 plays (76.3\%)
    \item The remaining 6,857 plays (23.7\%) were \textbf{inexcusable mistakes}
\end{itemize}

\begin{remark}
The 3.8\% of plays where models disagree represent situations where the data available at the time genuinely pointed to a different conclusion. For these plays, we cannot fault the coach for following the ex ante recommendation, even if ex post analysis suggests otherwise.
\end{remark}

\subsection{Stability Over Time}

Figure \ref{fig:time_trends} shows that the agreement rate is remarkably stable across seasons, ranging from 92.7\% (2019) to 97.4\% (2021). The 2019 dip likely reflects that season's unusual characteristics (including rule changes affecting play-calling), but even in this outlier year, over 92\% of decisions had stable optimal actions.
...
\begin{enumerate}
    \item Models trained on pre-season data yield recommendations that match full-sample analysis 96\% of the time
    \item The remaining 4\% of disagreements are concentrated in edge cases where even substantial additional data does not clearly resolve the optimal action
    \item Coaches could have---and should have---made better decisions using information that was publicly available
\end{enumerate}

This is not to say coaches have perfect information. They face execution uncertainty, opponent-specific adjustments, and time pressure that models do not capture. But the \textit{structural} uncertainty about whether going for it beats punting is largely resolved by historical data. The 24\% error rate reflects decision-making failures, not information failures.
...
Twenty years after \citet{romer2006} documented systematic conservatism, coaches still make suboptimal decisions approximately 24\% of the time. The average cost is 0.33 percentage points of win probability per fourth-down decision. Critically, our expanding window analysis shows that 96.2\% of optimal decisions were knowable in real-time---the recommendation using only pre-season data matches the full-sample recommendation. This undermines claims that coaches lacked sufficient information to make better decisions.
