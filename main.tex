...
\paragraph{Model validation.} Despite its simplicity, our win probability model correlates highly with \texttt{nflfastR}'s more sophisticated model ($r = 0.980$, $R^2 = 0.961$). Across 50,000 randomly sampled plays, the mean absolute difference is 4.19 percentage points. Stratified analysis reveals close agreement in close games (within 7 points, $r = 0.928$) and minimal systematic bias: we slightly underestimate WP in the final two minutes ($-3.0$ pp bias) where \texttt{nflfastR}'s end-of-game modeling is more sophisticated, but elsewhere the bias is under 1 percentage point. This validation ensures our results are not driven by WP model misspecification.
