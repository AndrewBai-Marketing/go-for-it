...
\begin{abstract}
We develop a Bayesian decision-theoretic framework for analyzing fourth down decisions in American football. Unlike existing approaches that rely on point estimates of win probability, our framework explicitly accounts for parameter uncertainty by integrating over posterior distributions of transition probabilities. We extend the baseline models using hierarchical Bayes to capture \textit{both} offensive team conversion ability and defensive team stopping ability, as well as kicker-specific field goal accuracy, with empirical Bayes shrinkage ensuring stable estimates. Win probability is estimated using a Bayesian logistic regression with 10 features including home field advantage, fitted via Laplace approximation. Following standard practice in sports analytics, we exclude plays with fewer than 60 seconds remaining where win probability models become unreliable. This allows us to make a selection-on-observables argument: by conditioning on the matchup-specific factors that coaches observe at decision time, we claim our model captures the coach's decision-relevant information set. Applying this framework to 71,786 fourth-down situations from 2006--2024, we find that coaches match model recommendations 75.9\% of the time. Using an expanding window methodology with a 7-year minimum training period starting from 1999, we show that 93.6\% of optimal decisions were \textit{knowable in real-time}. Critically, the ``analytics revolution'' increased league-wide go-for-it rates from 12.1\% to 20.0\% between 2006 and 2024---but the optimal rate is only 9.4\%. Coaches have \textit{overcorrected}, swapping conservative errors for aggressive ones. Teams now make 7,779 aggressive mistakes vs. 3,930 conservative mistakes. Despite this behavioral shift, decision quality has not improved, and teams lose an average of 0.27 expected wins per season from fourth down errors. League-wide WP loss has actually \textit{increased}: 8.2 wins in 2006 to 12.3 wins in 2024. The fact that coaches do not outperform model recommendations, and that playoff decisions are no better than regular season decisions despite higher stakes, supports our claim that coach suboptimality reflects behavioral biases rather than private information.
\end{abstract}
...
Notably, the ``Go Rate'' column reveals a dramatic behavioral shift: coaches increased their go-for-it rate from approximately 12\% (2006--2017) to 20\% (2020--2024). Yet this increased aggression did not improve decision quality---the ex ante optimal rate remained around 76\% throughout the period. The analytics revolution changed \textit{behavior} but not \textit{accuracy}. In fact, coaches are now \textit{too aggressive}---the optimal GO rate is only 9.4\%, but coaches go 14.8\%.
...
\paragraph{The paradox.} Coaches increased their go-for-it rate by 22 percentage points, but the model's recommended rate increased by only 2 percentage points. The result: coaches \textit{overcorrected}. Under-aggressive errors (failing to go when optimal) dropped from 20.5\% to 12.5\%, but over-aggressive errors (going when suboptimal) increased from 18.2\% to 30.6\%.
