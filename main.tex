...
2019 CLE @ PIT & 4th \& 2, own 46, down 1, 2:02 Q4 & GO & PUNT & 23.5\% \\
\bottomrule
\end{tabular}
\end{table}

\begin{remark}[Interpreting the Worst Decisions]
The costliest decisions share a common pattern: going for it from one's own territory in the final 2--3 minutes when trailing by 1--3 points. In these situations, the model strongly prefers punting: a failed conversion gives the opponent excellent field position to run out the clock or kick a field goal, while a successful punt forces the opponent to execute a longer drive with time pressure. The 23--27\% margins reflect the high leverage of these situations---a single decision can swing win probability by nearly a quarter.

Notably absent from this list is the 2021 Bills @ Titans game, where Buffalo went for it on 4th \& 1 from the Tennessee 3-yard line with 22 seconds remaining while trailing by 3. While earlier logistic regression models suggested a field goal was optimal, our neural network win probability model---which correctly captures goal-line dynamics in endgame scenarios---shows that going for it was actually the correct decision. The model estimates E[WP $|$ GO] = 54.6\% versus E[WP $|$ FG] = 44.8\%, reflecting the $\sim$67\% historical touchdown rate from 1st \& goal inside the 3-yard line. Josh Allen was stopped for no gain, but this was execution failure, not decision error.
\end{remark}
