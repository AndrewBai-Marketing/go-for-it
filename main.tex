...
\begin{abstract}
We develop a Bayesian decision-theoretic framework for analyzing fourth down decisions in American football. Unlike existing approaches that rely on point estimates of win probability, our framework explicitly accounts for parameter uncertainty by integrating over posterior distributions of transition probabilities. We extend the baseline models using hierarchical Bayes to capture \textit{both} offensive team conversion ability and defensive team stopping ability, as well as kicker-specific field goal accuracy, with empirical Bayes shrinkage ensuring stable estimates. Win probability is estimated using a Bayesian logistic regression with 10 features including home field advantage, fitted via Laplace approximation. Following standard practice in sports analytics, we exclude plays with fewer than 60 seconds remaining where win probability models become unreliable. This allows us to make a selection-on-observables argument: by conditioning on the matchup-specific factors that coaches observe at decision time, we claim our model captures the coach's decision-relevant information set. Applying this framework to 71,786 fourth-down situations from 2006--2024, we find that coaches match model recommendations only 52.5\% of the time. Using nflfastR's win probability model with proper state-transition calculations, we find that the optimal go-for-it rate is \textbf{46.6\%}---but coaches go only 14.8\%. On 4th \& 1, the optimal GO rate is \textbf{70.8\%}, yet coaches go only $\sim$25\%. Coaches remain \textit{systematically too conservative}. We validate these recommendations using Win Probability Added (WPA): when the model says GO and teams follow, they gain +0.0059 WPA; when the model says GO but teams don't, they lose value. On 4th \& 1 specifically, following GO recommendations yields +0.0125 WPA while ignoring them costs $-$0.0108 WPA. Teams lose an average of 0.27 expected wins per season from fourth down errors. The fact that coaches do not outperform model recommendations, and that playoff decisions are no better than regular season decisions despite higher stakes, supports our claim that coach suboptimality reflects behavioral biases rather than private information.
\end{abstract}
...
Recent work by \citet{brill2025} addresses the second limitation by incorporating bootstrap uncertainty quantification into fourth-down analysis, demonstrating that optimal decision recommendations are substantially more uncertain than sports analysts typically acknowledge. Our approach differs in two key respects: we adopt a fully Bayesian decision-theoretic framework that computes posterior expected win probability (rather than frequentist confidence intervals), and we implement expanding window estimation to assess what coaches could have known \textit{at the time of each decision}.
...
We now specify the complete hierarchical Bayesian model for fourth-down conversion. The model has three levels: (i) observations, (ii) unit-level parameters, and (iii) hyperparameters governing partial pooling.

\paragraph{Level 1: Observation model.} For observation $i$ with yards to go $d_i$, goal-to-go indicator $g_i$, in-game EPA $e_i$, drive play count $p_i$, offensive team $j[i]$, and defensive team $k[i]$, the outcome $y_i \in \{0, 1\}$ follows:
\begin{equation}
...
\paragraph{Level 2: Unit-level priors.} The team-specific effects are drawn from normal distributions centered at zero:
\begin{align}
...
\paragraph{Level 3: Hyperpriors and population parameters.} The population-level coefficients receive weakly informative priors:
\begin{equation}
...
\begin{equation}
    \hat{\tau}^2 = \max\left\{0, \text{Var}(\hat{\gamma}_j^{\text{raw}}) - \overline{\text{SE}^2}\right\}
\end{equation}
where $\hat{\gamma}_j^{\text{raw}}$ are the unpooled team effects and $\overline{\text{SE}^2}$ is the average squared standard error.

\paragraph{Joint likelihood.} The full data likelihood, conditional on all parameters, is:
\begin{equation}
...
\paragraph{Posterior distribution.} By Bayes' theorem, the joint posterior is:
\begin{equation}
...
\paragraph{Laplace approximation.} We approximate the posterior as Gaussian around the maximum a posteriori (MAP) estimate:
\begin{equation}
...
\paragraph{Empirical Bayes shrinkage.} Given the estimated $\hat{\tau}^2$, the shrinkage factor for unit $j$ with standard error $\text{SE}_j$ is:
\begin{equation}
...
\paragraph{Hierarchical punt model.} We model net punt yards as linear in field position with punter-specific effects:
\begin{equation}
    Y \mid x, \text{punter} = j \sim \mathcal{N}(\alpha + \beta x + \gamma_j, \sigma^2)
\end{equation}
where $\gamma_j \sim \mathcal{N}(0, \tau^2)$ captures punter ability relative to league average. The population-level estimates are $\hat{\alpha} = 32.8$ (SE: 0.41), $\hat{\beta} = 0.154$ (SE: 0.006), and $\hat{\sigma} = 9.3$ yards. The between-punter variance is $\hat{\tau}^2 = 1.84$, implying a punter standard deviation of 1.36 yards---the best punters add roughly 2--3 yards of net punt distance compared to average. Punts from deeper in own territory travel further (positive $\beta$), reflecting punter adjustment to field position.

\paragraph{Win probability model.} We model win probability using a Bayesian logistic regression trained on plays from 2006--2024. The model uses 10 features: score differential, time remaining (normalized), field position (normalized), timeout differential, score$\times$time interaction, time squared, late-game indicator ($\tau < 300$s), score$\times$late interaction, field position$\times$late interaction, and \textit{home field advantage}. The model is fitted via Laplace approximation, providing both point estimates and proper uncertainty quantification.

Key estimated coefficients include: score differential (3.38), time remaining ($-0.20$), score$\times$time interaction ($-2.18$), field position ($-0.44$), timeout differential (0.12), and home field advantage (0.33). The home field coefficient implies approximately a 2 percentage point WP advantage for the home team in neutral game states.

\paragraph{Model validation.} The Bayesian WP model is fitted via Laplace approximation at the MAP estimate, with uncertainty propagated through posterior draws. The model captures the key dynamics: score dominates late in games (large score coefficient, negative time coefficient), and home field provides a meaningful advantage (is\_home = 0.33). This calibration is critical for decision-making: we need accurate probability estimates that properly account for parameter uncertainty.
...
\midrule
2006 & 3,829 & 12.7\% & 81.8\% & 78.6\% & 93.6\% \\
2007 & 3,758 & 12.7\% & 78.9\% & 78.2\% & 95.6\% \\
2008 & 3,674 & 12.7\% & 80.3\% & 78.3\% & 95.6\% \\
2009 & 3,849 & 12.7\% & 79.0\% & 78.0\% & 96.3\% \\
2010 & 3,840 & 12.7\% & 82.5\% & 79.4\% & 94.9\% \\
2011 & 3,845 & 12.7\% & 82.0\% & 80.8\% & 95.2\% \\
2012 & 3,820 & 12.7\% & 82.6\% & 80.2\% & 96.1\% \\
2013 & 3,904 & 12.7\% & 83.8\% & 81.9\% & 97.0\% \\
2014 & 3,740 & 12.7\% & 81.6\% & 80.6\% & 96.3\% \\
2015 & 3,824 & 12.7\% & 79.7\% & 81.2\% & 93.6\% \\
2016 & 3,703 & 12.7\% & 82.1\% & 81.4\% & 98.2\% \\
2017 & 3,869 & 12.7\% & 83.0\% & 82.0\% & 98.5\% \\
2018 & 3,574 & 12.7\% & 81.8\% & 82.0\% & 97.3\% \\
2019 & 3,634 & 19.2\% & 80.0\% & 80.2\% & 99.1\% \\
2020 & 3,402 & 19.2\% & 80.2\% & 79.5\% & 96.8\% \\
2021 & 3,777 & 19.2\% & 78.9\% & 78.8\% & 96.0\% \\
2022 & 3,870 & 19.2\% & 79.5\% & 79.1\% & 97.4\% \\
2023 & 4,047 & 19.2\% & 79.0\% & 79.7\% & 96.1\% \\
2024 & 3,827 & 19.2\% & 79.1\% & 79.3\% & 98.0\% \\
\midrule
\textbf{Overall} & \textbf{71,786} & \textbf{14.8\%} & \textbf{75.9\%} & \textbf{72.7\%} & \textbf{93.6\%} \\
\bottomrule
\end{tabular}
\end{table}

The key finding: \textbf{93.6\% of plays have stable optimal decisions}. The ex ante model (trained only on data available before the season) and the ex post model (trained on all data through 2024) agree on the optimal action in the vast majority of cases. This finding is remarkably consistent across all 19 test years.

Notably, the ``Go Rate'' column reveals a behavioral shift: coaches increased their go-for-it rate from approximately 12\% (2006--2017) to 20\% (2020--2024). Yet coaches remain \textit{far too conservative}---the optimal GO rate using nflfastR's WP model is \textbf{46.6\%}, but coaches go only 14.8\%. On 4th \& 1, the gap is even starker: optimal GO rate is 70.8\%, but coaches go only $\sim$25\%. WPA analysis validates these recommendations: when the model says GO and teams follow, they gain +0.0059 WPA on average.
...
\midrule
Correct & 16,634 & 75.9\% & Coach agreed with model \\
Difference of opinion & 4,796 & 21.2\% & Margin $<$ 2\% WP \\
Questionable & 1,035 & 4.6\% & Margin 2--5\% WP \\
Clear mistake & 108 & 0.5\% & Margin $>$ 5\% WP \\
\midrule
\textbf{Total} & \textbf{22,573} & \textbf{100\%} & \\
\bottomrule
\end{tabular}
\end{table}

This decomposition reveals a striking fact: \textbf{80.8\% of ``mistakes'' are actually close calls}. When coaches deviate from model recommendations, they overwhelmingly do so in situations where the data do not clearly favor one action. Only 0.5\% of all fourth-down plays---and just 1.8\% of deviations---represent clear errors where the coach should unambiguously have known better.
...
\midrule
2021 BUF @ TEN & 4th \& 1, opp 3, down 3, 0:22 Q4 & GO & FG & 34.3\% \\
2024 CHI @ SEA & 4th \& 10, opp 40, down 3, 0:20 Q4 & GO & FG & 19.3\% \\
2009 --- & 4th \& 10, opp 40, tied, 0:19 Q4 & PUNT & FG & 14.6\% \\
2024 ATL @ NO & 4th \& 4, opp 43, down 3, 0:07 Q4 & GO & FG & 10.0\% \\
2019 TEN @ IND & 4th \& 2, opp 45, down 2, 0:13 Q4 & GO & FG & 9.6\% \\
\bottomrule
\end{tabular}
\end{table}

\begin{remark}[Interpreting the Worst Decisions]
The costliest decisions fall into two categories. First, going for it when a field goal would have tied or won the game: Buffalo's decision to go for it on 4th \& 1 at the Tennessee 3-yard line, down 3 with 22 seconds left, cost 34.3\% WP---a game-deciding mistake. Second, going for it or punting late in close games instead of attempting a makeable field goal: the remaining top decisions all involve teams passing up FG attempts in favor of low-probability conversions.

The maximum WP cost of 34.3\% reflects a true game-deciding situation where the alternative (a FG to tie) was clearly optimal. The hierarchical model's kicker-specific effects make FG recommendations more precise.
\end{remark}
...
CLE & 0.34 & 0.37 & 0.38 & 0.25 & 0.76 & \textbf{0.34} \\
NYJ & 0.12 & 0.19 & 0.25 & 0.69 & 0.51 & 0.33 \\
TEN & 0.31 & 0.32 & 0.31 & 0.40 & 0.31 & 0.33 \\
BAL & 0.18 & 0.17 & 0.34 & 0.37 & 0.10 & 0.32 \\
NYG & 0.27 & 0.36 & 0.20 & 0.14 & 1.18 & 0.32 \\
\midrule
\multicolumn{7}{l}{\textit{Most efficient decision-making:}} \\
DAL & 0.24 & 0.29 & 0.30 & 0.20 & 0.31 & \textbf{0.23} \\
NO & 0.17 & 0.25 & 0.15 & 0.31 & 0.27 & 0.22 \\
SF & 0.28 & 0.25 & 0.28 & 0.13 & 0.37 & 0.22 \\
GB & 0.46 & 0.28 & 0.43 & 0.22 & 0.21 & 0.24 \\
HOU & 0.25 & 0.27 & 0.29 & 0.29 & 0.20 & 0.24 \\
\midrule
League Avg & 0.27 & 0.30 & 0.34 & 0.34 & 0.37 & 0.27 \\
\bottomrule
\end{tabular}
\end{table}

The average team loses approximately \textbf{0.27 expected wins per season} from suboptimal fourth-down decisions. The spread between best (San Francisco/New Orleans, 0.22) and worst (Cleveland, 0.34) is 0.12 wins per season.

\paragraph{Putting the magnitude in perspective.} Is 0.27 wins per season meaningful? Consider:

\begin{itemize}
    \item \textbf{Playoff implications}: In the NFL's competitive balance environment, playoff berths are often decided by tiebreakers among teams with identical records. From 2020--2024, 23 teams missed the playoffs by one game. An additional 0.27 wins per season translates to roughly one extra playoff appearance per decade from fourth-down optimization alone.
    ...
    \item \textbf{Cumulative effect}: Over a 17-game season, the worst teams (Cleveland: 0.34 wins lost) squander roughly a third of a win from fourth-down mistakes alone. Over a 5-year coaching tenure, that's 1.7 expected wins---enough to flip multiple close games.
...
Notably, Cleveland, New York Jets, and Tennessee---teams with high variance in decision-making quality---appear among the ``most costly'' teams. This counterintuitive result reflects the nature of our calculation: aggressive teams \textit{attempt} more fourth-down conversions, creating more opportunities for deviation from model recommendations. A team that punts on every fourth down would show zero ``cost'' because they always match the model in obvious punt situations, but this would not indicate optimal decision-making.

\begin{remark}
The 3.4\% of plays where models disagree represent situations where the data available at the time genuinely pointed to a different conclusion. For these plays, we cannot fault the coach for following the ex ante recommendation, even if ex post analysis suggests otherwise.
\end{remark}
...
\paragraph{Coaches remain too conservative.} Despite increased aggression, teams continue to leave significant value on the table. The nflfastR WP model shows:
\begin{itemize}
    \item Optimal GO rate: 46.6\% vs actual 14.8\%---a 32 percentage point gap
    \item 4th \& 1 optimal GO rate: 70.8\% vs actual $\sim$25\%---teams go less than half as often as they should
    \item Average 0.27 expected wins lost per team per season from conservative errors
\end{itemize}

The dominant error remains excessive conservatism: teams punt or kick field goals when going for it would maximize win probability. WPA analysis validates this---when teams follow GO recommendations, they gain +0.0059 WPA; when they ignore them, they leave value on the table.

\paragraph{The analytics message is correct---coaches just aren't listening enough.} The popular takeaway from fourth-down research has been ``go for it more,'' and this is correct. The nflfastR WP model recommends going for it 46.6\% of the time overall, and 70.8\% on 4th \& 1. Coaches go only 14.8\% overall. The gap remains enormous. The WPA evidence is clear: following aggressive recommendations yields positive outcomes.

\paragraph{Free wins remain uncaptured.} Despite two decades of publicly available research showing that teams are systematically suboptimal on fourth down, the expected win probability cost has not declined. The ``free wins'' that \citet{romer2006} identified are still on the table. Teams remain far too conservative: the optimal GO rate is 46.6\%, but coaches go only 14.8\%. WPA analysis confirms the value left on the table---when models recommend GO but teams don't, they sacrifice +0.0059 WPA per play on average (+0.0125 WPA on 4th \& 1).

\subsection{Where Did Coaches Learn? A Margin-Based Decomposition}

The overall lack of improvement masks heterogeneous learning across decision types. We decompose decision quality by \textit{decision margin}---the win probability difference between the optimal and second-best action---to identify where coaches have learned and where they have not.

\begin{table}[H]
\centering
\caption{Decision Quality by Margin Size (2006--2024)}
\label{tab:margin_quality}
\begin{tabular}{@{}lcccc@{}}
\toprule
...
\paragraph{Coaches got worse on close calls.} On decisions where the margin is less than 2 percentage points---53\% of all fourth downs---optimal rates \textit{declined} by 0.17 pp/year. This is precisely the category where ``go for it more'' intuition provides little guidance, because the optimal action depends sensitively on game state.

\paragraph{No improvement on easy decisions.} On decisions with clear optimal actions (margin $>5$pp), coaches were already near-perfect (98.5\%) and stayed that way. There was no room to improve.

\subsubsection{The Short Yardage Paradox}

The most dramatic shift occurred in short yardage situations (4th \& 1--2), where the analytics revolution's message to ``go for it more'' landed hardest.

\begin{table}[H]
\centering
\caption{Short Yardage (1--2 yards): The Aggression Shift}
\label{tab:short_yardage_shift}
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Metric} & \textbf{Early (2006--2012)} & \textbf{Late (2019--2024)} & \textbf{Change} \\
\midrule
Coach go-for-it rate & 32.6\% & 54.8\% & +22.2 pp \\
Model go-for-it rate & 34.9\% & 36.7\% & +1.8 pp \\
Optimal decision rate & 60.9\% & 56.6\% & $-4.3$ pp \\
\midrule
Under-aggressive errors & 20.5\% & 12.5\% & $-7.9$ pp \\
Over-aggressive errors & 18.2\% & 30.6\% & +12.5 pp \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{The conservative gap persists.} Coaches increased their go-for-it rate by 22 percentage points over the sample period, yet they remain far too conservative. The nflfastR WP model recommends going for it 46.6\% of the time overall, and 70.8\% on 4th \& 1. Even with the increase in aggression, coaches go only 14.8\% overall. Conservative errors dominate: teams repeatedly punt or kick when going for it would maximize win probability.

\paragraph{Compliance by optimal action.} The asymmetry is stark:
\begin{itemize}
    \item When model says \texttt{go}: compliance improved 41\% $\to$ 66\% (+1.73 pp/year)
    \item When model says \texttt{punt}: compliance \textit{dropped} 85\% $\to$ 66\% ($-1.35$ pp/year)
    \item When model says \texttt{fg}: compliance dropped 47\% $\to$ 33\% ($-1.10$ pp/year)
\end{itemize}

Coaches learned the ``go for it'' half of the message while unlearning the ``but not always'' qualifier.

\subsubsection{Learning by Game Situation}

\begin{table}[H]
\centering
\caption{Learning Trends by Game Situation}
\label{tab:situation_trends}
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Situation} & \textbf{N Plays} & \textbf{Optimal \%} & \textbf{Trend (pp/yr)} \\
\midrule
\multicolumn{4}{l}{\textit{By time remaining}} \\
\quad End game ($<$5 min) & 8,641 & 60.0\% & $+0.16$ \\
\quad Early game ($>$30 min) & 35,466 & 86.0\% & $-0.22$ \\
\midrule
\multicolumn{4}{l}{\textit{By yards to go}} \\
\quad Short (1--2 yds) & 13,641 & 60.1\% & $-0.33$ \\
\quad Long (6--10 yds) & 23,867 & 88.2\% & $+0.00$ \\
\midrule
\multicolumn{4}{l}{\textit{By field position}} \\
\quad Red zone (0--20) & 11,943 & 69.3\% & $+0.25$ \\
\quad Midfield (40--60) & 17,361 & 83.0\% & $-0.37$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{High-stakes learning.} End-game decisions ($<5$ minutes remaining) show marginal improvement (+0.16 pp/year), while early-game decisions show decline ($-0.22$ pp/year). When stakes are salient, coaches pay closer attention.

\paragraph{Red zone improvement.} Red zone fourth downs show improvement (+0.25 pp/year), likely because these high-visibility situations receive more attention and are easier to track outcome feedback. Midfield decisions, where the tradeoffs are more complex, show notable decline ($-0.37$ pp/year).

\paragraph{Interpretation.} The pattern suggests coaches learn best in situations with: (i) immediate, clear feedback, (ii) high salience (late game, red zone), and (iii) simple decision structure. They struggle with: (i) delayed or noisy feedback, (ii) routine situations (early game), and (iii) complex tradeoffs (midfield, short yardage where all three options are viable).
...
Optimal decision rate & 75.9\% & 71.3\% & $-2.4$ pp \\
...
But the information environment is unchanged. Coaches don't suddenly learn their conversion probability better in January. The low accuracy rate reflects systematic conservatism bias, not fundamental uncertainty---coaches consistently underutilize the go-for-it option relative to what WP-maximizing analysis recommends.
...
The fact that our model achieves 93.6\% agreement between ex-ante and ex-post recommendations (Section~5) further supports the selection assumption: if private information mattered, we would expect more disagreement as the model learns from outcomes the coach couldn't observe.
...
\subsection{Coaches Are Learning Slowly (Unlike Fourth Down)}

We apply the same expanding window methodology used for fourth-down analysis: for each year, train hierarchical models on all \textit{prior} years only, then evaluate that year's decisions. This avoids data leakage and mirrors exactly what coaches could have known in real-time.

\textbf{Model specification:} The full hierarchical model includes:
\begin{itemize}[nosep]
    \item Two-point conversion model with offensive team effects ($\alpha_i^{\text{off}}$) and defensive team effects ($\alpha_j^{\text{def}}$)
    \item PAT model with kicker-specific effects ($\gamma_k$)
    \item Same Bayesian win probability model as fourth-down analysis
\end{itemize}

\begin{table}[H]
\centering
\caption{Two-Point Decision Quality Over Time (Expanding Window)}
\begin{tabular}{lccccc}
\toprule
\textbf{Season} & \textbf{N} & \textbf{Training Data} & \textbf{Optimal \%} & \textbf{Actual 2pt \%} & \textbf{PAT Success \%} \\
\midrule
2016 & 1,361 & 2015 only & 43.4\% & 8.3\% & 94.2\% \\
2017 & 1,283 & 2015--2016 & 51.3\% & 7.1\% & 93.9\% \\
2018 & 1,418 & 2015--2017 & 57.3\% & 9.7\% & 94.0\% \\
2019 & 1,384 & 2015--2018 & 56.1\% & 8.6\% & 94.1\% \\
2020 & 1,538 & 2015--2019 & 55.5\% & 9.3\% & 94.1\% \\
2021 & 1,481 & 2015--2020 & 55.8\% & 10.8\% & 93.9\% \\
2022 & 1,381 & 2015--2021 & 56.0\% & 9.3\% & 94.0\% \\
2023 & 1,370 & 2015--2022 & 58.2\% & 9.6\% & 94.1\% \\
2024 & 1,450 & 2015--2023 & 54.8\% & 10.2\% & 94.3\% \\
\midrule
\textbf{Trend} & & & \textbf{+1.05 pp/yr} & +0.29 pp/yr & \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item \textit{Note:} 2015 excluded from trend analysis as first post-rule year uses same-year data (data leakage). Training data column shows years used to fit hierarchical models before evaluating that season's decisions.
\end{tablenotes}
\end{table}

Unlike fourth down where we found no improvement in decision quality, coaches \textit{are} improving at PAT/2pt decisions---but slowly. The trend of $+1.05$ pp/year is modest but positive. Coaches started at $\sim$43\% optimal in 2016 (when training data was scarce) and have stabilized around $\sim$55\% optimal as more post-rule data accumulated.

\paragraph{Defensibly optimal decisions.} Given model uncertainty, we also track ``defensibly optimal'' decisions: those matching the model's recommendation \textit{or} having a WP margin below 0.5\% (genuinely too close to call). By this metric, coaches improved from 83\% (2016) to 99\% (2024), with a trend of $+2.32$ pp/year ($p < 0.001$). This suggests coaches have largely mastered the two-point decision environment---the remaining suboptimality is concentrated in situations where the model's recommendation is essentially a coin flip.
...
\caption{The two-point conversion paradox. Blue bars show the model-optimal rate at which coaches should go for 2; red bars show actual coach behavior. Despite Down 9 having a \textit{higher} optimal 2pt rate than Down 8 (91\% vs.\ 85\%), coaches comply 84\% of the time when down 8 but only 1\% when down 9. The difference: going for 2 when down 8 ties the game immediately, while going for 2 when down 9 sets up a future tying field goal.}
\label{fig:two_point_paradox}
\end{figure}

The paradox is striking (Figure~\ref{fig:two_point_paradox}): when down 8 (trailing by 2 after the touchdown), coaches go for 2 at a 79\% rate---close to the model-optimal rate of 85\%. But when down 9 (trailing by 3 after the touchdown), coaches go for 2 only 1\% of the time, despite an even higher 91\% optimal rate. In 107 situations where the model recommended going for 2 when down 9, coaches kicked the PAT in all but one.
...
\midrule
Coach optimality rate & 75.9\% & $\sim$55\% \\
Clear mistakes (\% of all) & 0.5\% & 0.007\% \\
Maximum WP margin & $>$30\% & $\sim$5\% \\
Team effects ($\tau$) & $\sim$0.15 & $\sim$0.03 \\
Learning trend (expanding window) & None ($-0.05$ pp/yr) & +1.05 pp/year \\
\bottomrule
\end{tabular}
\end{table}

Fourth-down decisions matter more (higher variance, larger WP margins) but coaches perform better at them. PAT/2pt decisions matter less, but coaches are worse at them---and slowly improving. This paradox suggests that decision complexity, not stakes, drives coaching errors: fourth down involves three options and subtle game-state interactions, while PAT/2pt is a binary choice with more straightforward probability tradeoffs.

\paragraph{Why the difference in learning rates?} We propose three explanations for why coaches are learning on two-point ($+1.2$ pp/yr) but not fourth down ($-0.05$ pp/yr):
\begin{enumerate}[nosep]
    \item \textbf{Decision simplicity}: Two-point is binary (go/kick), while fourth down has three options (go/punt/FG) with complex field position effects.
    \item \textbf{Clearer probability estimates}: Two-point success ($\sim$48\%) and PAT success ($\sim$94\%) are well-publicized and stable; conversion rates vary substantially by yards-to-go and context.
    \item \textbf{Lower stakes reduce overcorrection}: Because two-point margins are small ($<$2\% WP), coaches cannot hurt themselves much by experimenting, unlike fourth down where aggressive errors can cost 5--10\% WP.
\end{enumerate}
...
The comparison of regular season and playoff decision-making reveals that increased stakes shift aggressiveness but not accuracy. This is consistent with context-dependent risk aversion: playoff coaches require less of a margin to choose the aggressive option, but they still remain too conservative overall. The persistent gap between optimal GO rates (46.6\%) and actual GO rates (14.8\%) reflects systematic conservatism bias that coaches have not corrected despite two decades of analytics research.
...
\bibitem[Brill et al.(2025)]{brill2025}
Brill, R.~S., Yurko, R., \& Wyner, A.~J. (2025).
\newblock Analytics, have some humility: A statistical view of fourth-down decision making.
\newblock \textit{The American Statistician}, 79(3), 284--293.
