...
This limitation affects situations where: (1) the team is trailing by a one-possession margin, (2) a field goal would leave the opponent still leading, and (3) insufficient time remains for the opponent to be forced to play offense. We exclude such situations from the ``worst decisions'' table above, though they remain in aggregate statistics. Properly modeling end-of-game clock dynamics is an important extension for future work.
\end{remark}
