...
\begin{itemize}
    \item When model says \texttt{go}: compliance improved 41\% $\to$ 66\% (+1.73 pp/year, $p < 0.001$)
    \item When model says \texttt{punt}: compliance \textit{dropped} 85\% $\to$ 66\% ($-1.35$ pp/year, $p < 0.001$)
    \item When model says \texttt{fg}: compliance dropped 47\% $\to$ 33\% ($-1.10$ pp/year, $p = 0.001$)
\end{itemize}
...
\paragraph{High-stakes learning.} End-game decisions ($<5$ minutes remaining) show marginal improvement (+0.16 pp/year), while early-game decisions show decline ($-0.22$ pp/year, $p = 0.03$). When stakes are salient, coaches pay closer attention.

\paragraph{Red zone improvement.} Red zone fourth downs show improvement (+0.25 pp/year), likely because these high-visibility situations receive more attention and are easier to track outcome feedback. Midfield decisions, where the tradeoffs are more complex, show significant decline ($-0.37$ pp/year, $p < 0.001$).
