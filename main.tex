...
\midrule
\textbf{Trend} & & & \textbf{+1.2 pp/yr} & +0.29 pp/yr & \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
...
Unlike fourth down where we found no improvement in decision quality, coaches \textit{are} improving at PAT/2pt decisions---but slowly. The trend of $+1.2$ pp/year ($p = 0.02$) is modest but significant. Coaches started at $\sim$43\% optimal in 2016 (when training data was scarce) and have stabilized around $\sim$55\% optimal as more post-rule data accumulated. This likely reflects the simpler decision structure: with only two options and clear probability estimates, the ``right'' answer is easier to internalize over time.
...
Learning trend (expanding window) & None ($-0.05$ pp/yr) & +1.2 pp/year \\
\bottomrule
\end{tabular}
\end{table}
